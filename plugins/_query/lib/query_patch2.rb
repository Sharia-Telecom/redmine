  ...
  module QueryPatch
    ...
    def self.included(base)      
      ...
      base.class_eval do
        ...
        alias_method_chain :available_filters, :extra_filters
        alias_method_chain :sql_for_field, :extra_filters     
    ...
    module  InstanceMethods     
      def available_filters_with_extra_filters
        return @available_filters if @available_filters
        available_filters_without_extra_filters
 
        add_available_filter "and_any", :name => "Any follow (AND)", :type => :list, :values => ["Yes"]
        add_available_filter "or_any", :name => "Any follow (OR)", :type => :list, :values => ["Yes"]
        add_available_filter "or_all", :name => "All follow (OR)", :type => :list, :values => ["Yes"]
      ...
      def sql_for_field_with_extra_filters(field, operator, value, db_table, db_field, is_custom_filter=false)
          
        if ["^","!^"].include? operator 
          return sql_for_match_operators(field, operator, value, db_table, db_field, is_custom_filter)
        end
        
        return sql_for_field_without_extra_filters(field, operator, value, db_table, db_field, is_custom_filter)      
