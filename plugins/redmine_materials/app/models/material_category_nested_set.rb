module MaterialCategoryNestedSet
  ...
  def target_lft
    ...
    if siblings_rgt
      ...
      raise "MaterialCategory id=#{id} with parent_id=#{parent_id}: parent missing or without 'lft' value" unless parent_lft
      parent_lft + 1
    else
      1
    end
  ...
  def reload_nested_set_values
    self.lft, self.rgt = MaterialCategory.where(:id => id).pluck(:lft, :rgt).first
  end
  ...
  module ClassMethods
    ...
    def rebuild_nodes(parent_id = nil)
      nodes = MaterialCategory.where(:parent_id => parent_id).where(:rgt => nil, :lft => nil).reorder(:name)
